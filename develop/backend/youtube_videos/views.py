...
class Youtube_VideoDetail(APIView):
    ...
    # 쿠키에서 이미 조회한 비디오의 목록을 가져옴
        ...
        # 쿠키 설정
        expires = datetime.strftime(datetime.utcnow() + timedelta(days=30), "%a, %d-%b-%Y %H:%M:%S GMT")
        response.set_cookie("viewed_videos", ",".join(viewed_videos), expires=expires, httponly=True, secure=True, samesite='Lax')
